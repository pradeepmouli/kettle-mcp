...
describe('Execution Handlers', () => {
	...
	it('blocks execution if env var not set', async () => {
		...
		await expect(execHandlers.executeTransformation({ path: TEST_FILE })).rejects.toThrow(/not allowed/);
	});
	...
	it('blocks execution if file missing', async () => {
		await expect(execHandlers.executeTransformation({ path: '/tmp/does-not-exist.ktr' })).rejects.toThrow(/does not exist/);
	});

	it('throws for stopTransformation (not implemented)', async () => {
		await expect(execHandlers.stopTransformation({ path: TEST_FILE })).rejects.toThrow(/not implemented/);
	});
	...
	it('throws for stopJob (not implemented)', async () => {
		await expect(execHandlers.stopJob({ path: '/tmp/test.kjb' })).rejects.toThrow(/not implemented/);
	});
});
